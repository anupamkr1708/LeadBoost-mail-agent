...
def call_llm_json(
    ...
    *,
    ...
    """
    JSON-mode completion. The system prompt MUST instruct the model to
    return only JSON (enforced again here via Groq's response_format).
    Strips ```json fences defensively in case the model adds them anyway.
    """
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": human_prompt},
    ...
    try:
        client = _get_client()
        response = _chat_json(client, messages, temperature, max_tokens)
...
def _chat_json(client, messages, temperature, max_tokens) -> str:
    response = client.chat.completions.create(
        model=settings.llm_model,
        messages=messages,
        temperature=temperature if temperature is not None else settings.llm_temperature,
